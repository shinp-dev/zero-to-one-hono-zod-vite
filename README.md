...
各回に **開始時 (`start/`)** と **終了時 (`end/`)** の実プロジェクトを用意しています。

- [チェックポイント一覧](checkpoints/README.md)
- `session-N/end` は原則として `session-(N+1)/start` と同じ実装状態です
- 各フォルダは単体で `npm install` → `npm run dev` できる構成です
- 第6回以降は復旧用としてローカルD1のschemaを `predev` で自動適用します

途中でコードを壊した場合、欠席後に途中回から再開する場合、完成例との差分を確認したい場合に使います。通常の授業では、チェックポイントを丸写しするのではなく各Stepを自分で進めてください。
